...
public class Car {
    ...
    private void addToGraphics() {
        for(int i = 0; i < this.length; ++i) {
            if(this.leftToRight == 1) {
                this.game.getGraphic().add(new Element(this.leftPosition.absc + i, this.leftPosition.ord - this.game.score + 3,  this.colorLtR));
            }
            if(this.leftToRight == 0){
                this.game.getGraphic().add(new Element(this.leftPosition.absc + i, this.leftPosition.ord - this.game.score + 3,  this.colorRtL));
            }
            /*
            si le score augmente, la ligne superieur diminue proportiellement au score.
            Donc si le score augmente de 1, la ligne contenant la voiture diminue de 1
             */
        }
