...
public class Lane3{
    ...
    public Lane3(Game3 game, int ordonne, double density, boolean car) {//constructeur de la route avec une densite speciale
        this.game = game;
        this.ord = ordonne;
    ...
    public Lane3(Game3 game, int ord, boolean x) {//constructeur de la route avec une densité par defaut
    ...
    private void addToGraphicss(Color x) {
        for (int i = 0; i < this.game.width; i++) {
            Color color = x;
            game.getGraphic().add(new Element(i, this.ord - this.game.score+3, x));
        }
    }
    ...
    public void update() {
        ...
        if(this.car == true) {
            if(this.game.score < 3) {
                ...
                this.addToGraphicss(new Color(0x9A9A19));
            }
            ...
            if(this.game.score < 3) {
                ...
                this.addToGraphicss(Color.BLUE);
            }
            if (this.tick < this.speed) {
        ...
        if(this.density == 0){
            if(this.game.getFrog().getPosition().ord < 3) {
                this.addToGraphics(Color.LIGHT_GRAY);
            }else{
                this.addToGraphicss(Color.LIGHT_GRAY);
            }
        }
    }
    ...
    private void enleveRaft(){
        for(int i = 0; i < this.rafts.size(); i++){
            if(this.rafts.get(i).estPasDansLaGrille3()){
    ...
    private void enleveVoiture(){ //enlever la voiture du tableau de voiture si elle n'est plus dans la grille
        for(int i = 0; i < this.cars.size(); i++){
            if(this.cars.get(i).estPasDansLaGrille3()){
    ...
    public boolean contientObjet(Case3 c){ //methode qui retourne un bouleen pour savoir si la case 'c' dans la route contient un arbre ou une toile
    ...
    public void addCoin() { //methode pour rajouter une piece dans la route
        while(this.coin.size() < 1) {
            Case3 c = new Case3(this.genererInt(0, this.game.width), this.ord);
