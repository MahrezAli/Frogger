...
public class Lane2{
    ...
    public Lane2(Game2 game, int ordonne, double density, boolean car) {//constructeur de la route avec une densite speciale
        this.game = game;
        this.ord = ordonne;
    ...
    public Lane2(Game2 game, int ord, boolean x) {//constructeur de la route avec une densité par defaut
    ...
    public void isInRaft(boolean x){
        for(int i = 0; i < this.rafts.size(); i++){
            if(this.leftToRight == 0) {//rouge
                ...
                if (this.rafts.get(i).estDansLaCase(this.game.getFrog2().getPosition())) {
                    this.game.getFrog2().setLeftToRight(this.leftToRight);
                    this.game.getFrog2().deplacer(x);
                }
            }
            if(this.leftToRight == 1) {//rose
                ...
                if (this.rafts.get(i).estDansLaCases(this.game.getFrog2().getPosition())) {
                    this.game.getFrog2().setLeftToRight(this.leftToRight);
                    this.game.getFrog2().deplacer(x);
                }
            }
    ...
    private void enleveRaft(){
        for(int i = 0; i < this.rafts.size(); i++){
            if(this.rafts.get(i).estPasDansLaGrille2()){
    ...
    private void enleveVoiture(){ //enlever la voiture du tableau de voiture si elle n'est plus dans la grille
        for(int i = 0; i < this.cars.size(); i++){
            if(this.cars.get(i).estPasDansLaGrille2()){
    ...
    public boolean positionToile(){// si la grenouille est dans la toile
        if(this.car == true) {
            if (this.toile.size() > 0) {
                if (this.toile.get(0).estDansLaCase(this.game.getFrog().getPosition()) || this.toile.get(0).estDansLaCase(this.game.getFrog2().getPosition())) {
                    return true;
                }
            }
        }
    ...
    public boolean positionCoin2(){// si la grenouille est dans la piece
        if(this.coin.size() > 0){
            if(this.coin.get(0).estDansLaCase(this.game.getFrog2().getPosition())){
                return true;
            };
        }
        return false;
    ...
    public boolean contientObjet(Case c){ //methode qui retourne un booleen pour savoir si la case 'c' dans la route contient un arbre ou une toile
